...
beforeEach(async () =>{
    const {collection} = await connectMongo('banco_de_dados','news');
...
test('GET ONE - Correto - expect status 200', async()=>{
    const result = await agent.get(`/news/${newId}`)
    expect(result.statusCode).toBe(200)
    expect('object').toBe(typeof result.body)
    expect([result.body].length).toBe(1)
...
test('GET ONE - ID inválido - expect status 400', async()=>{
    const result = await agent.get('/news/a')
...
test('GET ONE - Usuário não cadastrado- expect status 404', async()=>{
    const result = await agent.get('/news/62606f034cb53af8dcdace96')
...
test('CREATE - Correto - expect status 201', async()=>{
    const result = await agent.post('/news').send({title:RandomTitleC, body:RandomBodyC})
    expect(result.statusCode).toBe(201)
    expect('object').toBe(typeof result.body)
    expect([result.body].length).toBe(1)
...
test('CREATE - Title vazio - expect status 400', async()=>{
    const result = await agent.post('/news').send({title:" ",body:RandomBodyC})
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('CREATE - Body vazio - expect status 400', async()=>{
    const result = await agent.post('/news').send({title:RandomTitleC,body:""})
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('UPDATE - Correto - expect status 200', async()=>{
    const result = await agent.put(`/news/${newId}`).send({title:RandomTitleC, body:RandomBodyC})
    expect(result.statusCode).toBe(200)
    expect('object').toBe(typeof result.body)
    expect([result.body].length).toBe(1)
    expect(result.body).toEqual({
...
test('UPDATE - Nome vazio - expect status 400', async()=>{
    const result = await agent.put(`/news/${newId}`).send({title:"", body:RandomBodyC})
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('UPDATE - Body vazio - expect status 400', async()=>{
    const result = await agent.put(`/news/${newId}`).send({title:RandomTitleC, body:" "})
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('UPDATE - ID inválido - expect status 400', async()=>{
    const result = await agent.put('/news/a').send({title:RandomTitleC, body:RandomBodyC})
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('UPDATE - Usuário não cadastrado - expect status 404', async()=>{
    const result = await agent.put('/news/62606f034cb53af8dcdace96').send({title:RandomTitleC, body:RandomBodyC})
    expect(result.statusCode).toBe(404)
    expect('object').toBe(typeof(result.body))
    expect([result.body.MessageError].length).toBe(1)
    expect(result.body.MessageError).toEqual("Usuário não encontrado")
...
test('DELETE - CORRETO - expect status 200', async()=>{
    const result = await agent.delete(`/news/${newId}`)
    expect(result.statusCode).toBe(200)
    expect('object').toBe(typeof result.body)
    expect([result.body].length).toBe(1)
    expect(result.body).toEqual({
...
test('DELETE - ID inválido - expect status 400', async()=>{
    const result = await agent.delete('/news/a')
    expect(result.statusCode).toBe(400)
    expect(true).toBe(Array.isArray(result.body.errors))
    expect(result.body.errors.length).toBe(1)
    expect(result.body.errors).toEqual([
...
test('DELETE - Usuário não cadastrado - expect status 404', async()=>{
    const result = await agent.delete('/news/62606f034cb53af8dcdace96')
    expect(result.statusCode).toBe(404)
    expect('object').toBe(typeof(result.body))
    expect([result.body.MessageError].length).toBe(1)
    expect(result.body.MessageError).toEqual("Usuário não encontrado")
